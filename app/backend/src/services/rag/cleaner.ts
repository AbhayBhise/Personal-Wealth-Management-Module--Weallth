 ...
 */
export function cleanAIResponseOutput(text: string, isDeveloperDebugMode: boolean = false): string {
  if (!text) return '';
  ...
  // 2. Hide Sources section unless Debug mode is active
  ...
  // 3. Strip internal chunk IDs and filenames if leaked
  cleaned = cleaned
    .replace(/dwwy_chunk_\d+/g, '')
    .replace(/rag_knowledge\.json/g, '')
    .replace(/book_chunks\.ts/g, '');

  // 4. Normalize Indian Rupee (₹) formatting (e.g. Rs. 25000 -> ₹25,000, INR 25000 -> ₹25,000)
  cleaned = cleaned
  ...
  // 6. Clean multiple empty line breaks
